            ...
            <div class="col-md-5 col-sm-5 hidden-xs left-col eq-height-col" >
                <div class="left-col-content login-header"> 
                    <div style="margin-top: 50%;">
                        {{--
                    <a href="/">
                    @if(file_exists(public_path('website/images/logo_test.png'))))
                        <img src="{{asset('public/website/images/logo_test.png')}}" class="img-rounded" alt="Logo" width="150">
                    @else
                    ...
                    <select class="form-control input-sm" id="change_lang" style="margin: 10px;">
                        @if(! empty($languages)) 
                            @foreach($languages as $key => $val)
                                <option value="{{$key}}" @if( (empty(request()->lang) && config('app.locale') == $key) || request()->lang == $key) selected @endif > {{$val['full_name']}} </option>
                            @endforeach
                        @else
                ...
                <div class="col-md-9 col-xs-8" style="text-align: right;padding-top: 10px;">
                    @if(!($request->segment(1) == 'business' && $request->segment(2) == 'register'))
                        ...
                        @if(config('constants.allow_registration'))
                            <a href="{{ route('register') }}@if(!empty(request()->lang)){{'?lang=' . request()->lang}} @endif" class="btn bg-maroon btn-flat" style="margin: 0 10px;"><b>{{ __('business.not_yet_registered')}}</b> {{ __('business.register_now') }}</a>
                            <!-- pricing url -->
