            ...
            <div class="collapse navbar-collapse px-0" id="bs-example-navbar-collapse-2">
                <div class="navbar-nav">
                    @if (auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only'))
                        <a href="{{action('SellController@index_company')}}" @if(request()->segment(1) == 'sellss' && request()->segment(2) == 'companys') class="active nav-link" @else class="nav-link" @endif><i class="fas fa-file-archive me-2"></i> {{__('sale.sells_company')}}</a>
                    @endif
                    {{--
                    @if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access'))
                        <a href="{{action('SellController@create')}}" @if(request()->segment(1) == 'sells' && request()->segment(2) == 'create') class="active nav-link" @else class="nav-link" @endif><i class="fas fa-plus-circle me-2"></i> {{__('sale.add_sale')}}</a>
                    @endif--}}
                    @if (auth()->user()->can('sell.create'))
                        <a href="{{action('SellController@draft_company')}}" @if(request()->segment(1) == 'sellss' && request()->segment(2) == 'draft') class="active nav-link" @else class="nav-link" @endif><i class="fas fa-pen-square me-2"></i> {{__('lang_v1.list_drafts')}}</a>
                        <a href="{{action('SellController@Quotations_company')}}" @if(request()->segment(1) == 'sellss' && request()->segment(2) == 'quotations') class="active nav-link" @else class="nav-link" @endif><i class="fas fa-clipboard-list me-2"></i> {{__('lang_v1.list_quotations')}}</a>
                    @endif
                </div>
            </div>
        </div>
