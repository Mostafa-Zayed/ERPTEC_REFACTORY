...
use App\Notifications\WelcomeBusiness;
...
class BusinessController extends Controller
{
    ...
    */
    ...
    protected $businessLocationInterface;
    ...
    public function __construct(
        ...
        BusinessLocationInterface $businessLocationInterface,
    ...
    )
    {
        ...
        $this->themeColors = config('business.themeColors');
        $this->mailDrivers = config('business.mailDrivers');
    ...
    //             $business = $this->businessInterface->createNewBusiness($request,$user);
    ...
    //             $this->businessInterface->newBusinessDefaultResources($business->id, $user->id);
                
    //             $businessLocation = $this->businessInterface->getBusinessLocationDetails($request);
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            Mail::to($user->email)->send(new BusinessRegistered());
            $output = ['success' => 1,
                    'msg' => __('business.business_created_succesfully')
                ];
    ...
    {
        return BusinessServie::searchWithColumnValue('username',$request->input('username'));
    }
    /**
    ...
    {
        if(BusinessService::businessCan(['business_settings.access'])){
            ...
            // $commission_agent_dropdown = BusinessService::getCommissionAgentDropdown();
            
            // $units_dropdown = Unit::forDropdown($business_id, true);

            $date_formats = config('business.date_formats');
            
            $shortcuts = json_decode($business->keyboard_shortcuts, true);
        
            $pos_settings = empty($business->pos_settings) ? config('business.defaultPosSettings') : json_decode($business->pos_settings, true);

            $email_settings = empty($business->email_settings) ? config('business.defaultEmailSettings') : $business->email_settings;
    
            $sms_settings = empty($business->sms_settings) ? config('business.defaultSmsSettings') : $business->sms_settings;
            
            $modules = Util::availableModules();
    ...
    public function storeBusiness(BusinessRequest $request)
    {
        try {
            ...
            $owner_details['password'] = Hash::make($owner_details['password']);
            
            $user = User::create($owner_details);
           
            $business_details = $request->only(['name', 'currency_id', 'time_zone']);

            $business_location = $request->only(['name', 'country']);
            ...
            event('business.created',$business);
            
            $new_location = $this->businessLocationInterface->addLocation($business->id,$business_location);

            Permission::create(['name' => 'location.' . $new_location->id ]);
            
            \Notification::route('mail', "mostafa.zayed9990@gmail.com")->notify(new WelcomeBusiness());
            ...
            $is_installed_superadmin = $this->moduleInterface->isInstalled('Superadmin');

            $package_id = ! empty($request->package_id) ? $request->get('package_id') : 2;
    ...
    private static function getEmailCredentials()
    {
