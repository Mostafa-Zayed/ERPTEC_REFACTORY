...
class ShopServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        view::composer('shop::layouts.partials.sidebar', function ($view) {
            $module_util = new ModuleUtil();

            if (auth()->user()->can('superadmin')) {
                $__is_woo_enabled = $module_util->isModuleInstalled('Shop');
            } else {
                $business_id = session()->get('user.business_id');
                $__is_woo_enabled = (boolean)$module_util->hasThePermissionInSubscription($business_id, 'shop_module', 'superadmin_package');
            }
