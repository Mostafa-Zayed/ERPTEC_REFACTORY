        ...
        <div class="container-fluid p-0">
            ...
            <div class="collapse navbar-collapse px-0 mx-0" id="bs-example-navbar-collapse-1">
                <div class="navbar-nav">
                    <a href="{{action('SearchSellController@index_test')}}" 
                        @if(
                            (request()->segment(1) == 'sellss' && request()->segment(2) == 'advance-sell') || (request()->segment(1) == 'sells-return' && request()->segment(2) == 'partsells')
                         || (request()->segment(1) == 'sells' && request()->segment(2) == 'drafts')
                         || (request()->segment(1) == 'import-sales' && request()->segment(2) == '') || (request()->segment(1) == 'sells' && request()->segment(2) == 'quotations')
                         || (request()->segment(1) == 'sell-return' && request()->segment(2) == '') || (request()->segment(1) == 'sellss' && request()->segment(2) == 'cancel')
                         || (request()->segment(1) == 'sellss' && request()->segment(2) == 'done') || (request()->segment(1) == 'sellss' && request()->segment(2) == 'search')
                         || (request()->segment(1) == 'sells-step' && request()->segment(2) == 'order' && request()->segment(2) == 'allsells') || (request()->segment(1) == 'sellss' && request()->segment(2) == 'import')
                         ) class="active-dark nav-link" 
                        @else class="nav-link" @endif><i class="fa fas fa-dollar-sign"></i> {{__('sale.sale')}}</a>
                    {{--@if (in_array('retail_sale', $enabled_modules))--}}
                        @if (auth()->user()->can('direct_sell.access') ||  auth()->user()->can('retail.view'))
                            <a href="{{action('SellController@index_retailer')}}" @if(request()->segment(2) == 'retail' || request()->segment(3) == 'retail') class="active-dark nav-link" @else class="nav-link" @endif><i class="fas fa-truck-loading me-2"></i> {{__('sale.retail_sale')}}</a>
                        @endif
                    {{--@endif--}}
                    @if (auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only'))
                        <a href="{{action('SellStepController@index_confirmed')}}" @if(request()->segment(1) == 'sells-step' || (request()->segment(3) == 'return' && request()->segment(4) =='recieve')) 
                            class="active-dark nav-link" @else class="nav-link" @endif><i class="fas fa-hourglass-start me-2"></i> {{__('sale.order_stages')}}</a>
                        <a href="{{action('SellController@index_company')}}"  
                            @if(request()->segment(2) == 'companys' || (request()->segment(1) == 'sells' && request()->segment(2) == 'create')
                                 || (request()->segment(1) == 'sellss' && request()->segment(2) == 'draft') || (request()->segment(1) == 'sellss' && request()->segment(2) == 'quotations')) 
                                class="active-dark nav-link" @else class="nav-link" @endif><i class="fas fa-file-alt me-2"></i> {{__('sale.company_sale')}}</a>
                        @if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only'))
                            @if (auth()->user()->can('sell_booking'))
                                <a href="{{action('SellController@indexbooking')}}" @if(request()->segment(1) == 'sellss' && request()->segment(2) == 'booking') class="active-dark nav-link" @else class="nav-link" @endif><i class="fas fa-list me-2"></i> {{__('lang_v1.all_sales_booking')}}</a>
                            @endif
                        @endif
                    @endif
                </div>
        ...
        <div class="dropdown nav-dropdown mb-4 me-3">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                ...
                @if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only'))
                    ...
                    @endif
                @endif
                 <a href="{{action('TransactionTypeController@index')}}" @if(request()->segment(1) == 'transaction-types') class="active-light dropdown-item" @else class="dropdown-item" @endif><i class="far fa-check-square"></i>{{__('sale.trans_types')}}</a>
            </div>
        </div>
