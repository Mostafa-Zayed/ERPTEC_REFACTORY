...
class DataController extends Controller
{
    ...
    {
        Artisan::call('db:seed', ["--class" => 'Modules\Shopify\Database\Seeders\AddDummySyncLogTableSeeder']);
    }
    ...
    public function superadmin_package()
    {
        return [
            [
                'name' => 'shopify_module',
                'label' => __('shopify::lang.shopify_module'),
                'default' => false
            ]
        ];
    }
    ...
    {
        $path = 'shopify::partials.product_form_part';

        $business_id = request()->session()->get('user.business_id');

        $module_util = new ModuleUtil();
        $is_woo_enabled = (boolean)$module_util->hasThePermissionInSubscription($business_id, 'shopify_module', 'superadmin_package');
        if ($is_woo_enabled) {
            return  [
                'template_path' => $path,
                'template_data' => []
    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_woo_enabled && (auth()->user()->can('woocommerce.syc_categories') || auth()->user()->can('woocommerce.sync_products') || auth()->user()->can('woocommerce.sync_orders') || auth()->user()->can('woocommerce.map_tax_rates') || auth()->user()->can('woocommerce.access_woocommerce_api_settings'))) {
            Menu::modify('admin-sidebar-menu', function ($menu) {
                $menu->url(
                    action('\Modules\Shopify\Http\Controllers\ShopifyController@index'),
                    __('shopify::lang.shopify'),
                    ['icon' => 'fab fa-wordpress', 'style' => config('app.env') == 'demo' ? 'background-color: #9E458B !important;' : '', 'active' => request()->segment(1) == 'shopify']
                )->order(150);
            });
        }
    }
