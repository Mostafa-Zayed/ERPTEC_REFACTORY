...
class BusinessUtil extends Util
{
    ...
    {
        $business_details['sell_price_tax'] = config('business.sell_price_tax'); //'includes';

        $business_details['default_profit_percent'] = config('business.default_profit_percent'); //25;

        //Add POS shortcuts
        $business_details['keyboard_shortcuts'] = config('business.keyboard_shortcuts'); //'{"pos":{"express_checkout":"shift+e","pay_n_ckeckout":"shift+p","draft":"shift+d","cancel":"shift+c","edit_discount":"shift+i","edit_order_tax":"shift+t","add_payment_row":"shift+r","finalize_payment":"shift+f","recent_product_quantity":"f2","add_new_product":"f4"}}';


        //Add prefixes
        $business_details['ref_no_prefixes'] = config('business.ref_no_prefixes'); 
        // [
        //     'purchase' => 'PO',
        //     'stock_transfer' => 'ST',
        //     'stock_adjustment' => 'SA',
        ...
        $business_details['enable_inline_tax'] = config('business.enable_inline_tax'); //0;
        dd($business_details);
        $business = Business::create($business_details); //create_business($business_details);
        ...
        if(!empty($location_details['city'])) {
            ...
            $location = BusinessLocation::create(['business_id' => $business_id,
                           ...
                           /* 'landmark' => $location_details['landmark'],*/
                            ...
                            'state' => $location_details['state'],
                            'zip_code' => !empty($location_details['zip_code']) ? $location_details['zip_code'] : '',
                            'country' => $location_details['country'],
                            'invoice_scheme_id' => $invoice_scheme_id,
                            'invoice_layout_id' => $invoice_layout_id,
                            'mobile' => !empty($location_details['mobile']) ? $location_details['mobile'] : '',
                            'alternate_number' => !empty($location_details['alternate_number']) ? $location_details['alternate_number'] : '',
                            'website' => !empty($location_details['websites']) ? $location_details['websites'] : '',
                            'email' => '',
                            'location_id' => $location_id,
                            'default_payment_accounts' => json_encode($location_payment_types)
                        ]);
